...
// Load invoice from JSON/CSV/Excel
document.getElementById("invoiceUpload").addEventListener("change", function (e) {
  ...
  if (ext === "json") {
    ...
    alert("Unsupported file format. Please upload JSON, CSV, or XLSX.");
  }
});
...
function populateInvoice(data) {
  ...
  data.forEach((item, i) => {
    ...
    row.innerHTML = `
      <td>${i + 1}</td>
      <td><input type="text" value="${item.name || item.Item || ""}"></td>
      <td><input type="text" value="${item.material || item.Material || ""}"></td>
      <td><input type="number" class="price" value="${item.price || item.Price || 0}"></td>
    `;
  });
  calculateTotal();
}
