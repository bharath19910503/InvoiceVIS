...
document.getElementById("addItemBtn").addEventListener("click", () => {
    ...
    row.innerHTML = `
        ...
        <td>0</td>
    `;
    row.cells[1].addEventListener("input", calculateTotals); // Material Used
...
function calculateTotals() {
    ...
    invoiceTable.querySelectorAll("tr").forEach(row => {
        let materialUsed = parseFloat(row.cells[1].textContent) || 0;
        let rate = parseFloat(row.cells[2].textContent) || 0;
        let cost = materialUsed * rate;
...
document.getElementById("downloadInvoiceBtn").addEventListener("click", () => {
    ...
    doc.text("Address: NO 39 BRN Ashish Layout Near Sri Thimmaraya Swami Gudi Anekal - 562106", 14, footerY);
    doc.text("Phone: +91 9916511599 & +91 8553608981", 14, footerY + 8);
    doc.text("Email: Varshithinteriorsolutions@gmail.com", 14, footerY + 16);
    doc.text("Note: 50% advance, 30% after 50% work completion, 20% on project completion.", 14, footerY + 30);
